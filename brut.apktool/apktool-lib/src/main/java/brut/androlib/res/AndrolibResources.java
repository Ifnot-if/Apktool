...
//        LOGGER.info("Loading resource table from file: " + apk);
    ...
    public void decodeManifestWithApplication(ResTable resTable, ExtFile apkFile, File outDir, AppInfo appInfo)
        throws AndrolibException {
        ...
        try {
            ...
            fileDecoder.decodeManifestApplication(inApk, "AndroidManifest.xml", out, "AndroidManifest.xml",appInfo);
        } catch (DirectoryException ex) {
            throw new AndrolibException(ex);
        }
    }

    public void decodeArsc(ResTable resTable, ExtFile apkFile, File outDir)
        throws AndrolibException {
        ...
        for (ResPackage pkg : resTable.listMainPackages()) {
            ...
            for (ResValuesFile valuesFile : pkg.listValuesFiles()) {
                if ("values/strings.xml".equals(valuesFile.getPath())
                    || "values/drawables.xml".equals(valuesFile.getPath())) {
                    generateValuesFile(valuesFile, out, xmlSerializer);
                    if ("values/drawables.xml".equals(valuesFile.getPath())) {
//                        LOGGER.info("ICON_NAME1:" + Global.ICON_NAME);
                        String name = ResXmlPatcher.getValueFromDrawables(outDir, Global.ICON_NAME);
                        if (name != null && !"".equals(name)) {
                            String[] ss = name.split("/");
                            Global.ICON_NAME = ss[ss.length - 1];
                        }
//                        LOGGER.info("ICON_NAME2:" + Global.ICON_NAME);
                    }
                }
            }
...
//            LOGGER.info("Decoding file-resources...");
            for (ResResource res : pkg.listFiles()) {
                switch (res.getResSpec().getType().getName()) {
                    case "drawable":
                    case "mipmap":
                        fileDecoder.decodeArsc(res, in, out, mResFileMapping);
                        break;
                }
            }
        }
